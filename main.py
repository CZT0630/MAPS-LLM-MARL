...
def _apply_overrides(config: dict, episodes: int | None, steps: int | None) -> dict:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--mode",
        choices=("train", "smoke", "audit"),
        default="smoke",
        help="train one baseline, run a short smoke test, or execute Gate 1 audit",
    )
    parser.add_argument(
        "--algorithm",
...
def main() -> int:
    ...
    config = _apply_overrides(
        load_config(str(config_path)), args.episodes, args.steps
    )

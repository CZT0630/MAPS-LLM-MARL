...
from .happo_policy import HAPPOActor, HAPPOCritic
...
class HAPPOAgent:
    def __init__(self, state_dim, action_dim, global_state_dim, agent_idx,
        ...
        self.kl_coeff = kwargs.get('kl_coeff', config.get('kl_coeff', 0.5))
        ...
        self.actor = HAPPOActor(
            state_dim,
            self.num_edges,
            partition_concentration=self.partition_concentration,
        ).to(self.device)
        self.critic = HAPPOCritic(global_state_dim).to(self.device)
    ...
    def _compute_policy_loss(self, batch):
        ...
        advantages = torch.as_tensor(batch['advantages'], dtype=torch.float32,
                                     device=self.device)
        partition_params_old = torch.as_tensor(
            batch['partition_params'], dtype=torch.float32, device=self.device
        )
        edge_logits_old = torch.as_tensor(
            batch['edge_logits'], dtype=torch.float32, device=self.device
        )
        ...
        part_dist, edge_dist, _, edge_logits_new, concentration_new = (
            self.actor.get_dist(states)
        )
        ...
        old_part_dist = Dirichlet(partition_params_old)
        old_edge_dist = Categorical(logits=edge_logits_old)
        kl_part = kl_divergence(old_part_dist, Dirichlet(concentration_new))
        kl_edge = kl_divergence(
            old_edge_dist, Categorical(logits=edge_logits_new)
        )
        kl = (kl_part + kl_edge).mean()
        entropy_part = part_dist.entropy().mean()
        entropy_edge = edge_dist.entropy().mean()
        entropy = entropy_part + entropy_edge

        loss = ppo_loss + self.kl_coeff * kl - self.entropy_coeff * entropy
        return loss, {

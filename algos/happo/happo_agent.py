...
from .happo_policy import HAPPOActor, HAPPOCritic, beta_kl_divergence
...
class HAPPOAgent:
    def __init__(self, state_dim, action_dim, global_state_dim, agent_idx, config=None, **kwargs):
        ...
        self.actor = HAPPOActor(state_dim, action_dim).to(self.device)
    ...
    def _compute_policy_loss(self, batch):
        ...
        alpha_old = torch.as_tensor(batch['alpha'], dtype=torch.float32, device=self.device)
        beta_old = torch.as_tensor(batch['beta'], dtype=torch.float32, device=self.device)

        advantages = (advantages - advantages.mean()) / (advantages.std() + 1e-8)

        dist_new, alpha_new, beta_new = self.actor.get_dist(states)
        ...
        kl = beta_kl_divergence(alpha_old, beta_old, alpha_new, beta_new).mean()
